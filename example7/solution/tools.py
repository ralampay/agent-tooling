...
@tool
def append_audit_log(packet: dict) -> str:
    """Append a review packet to an audit log file."""
    log_path = Path(__file__).parent / "data" / "audit_log.txt"
    log_path.write_text(log_path.read_text() + f"{packet}\n" if log_path.exists() else f"{packet}\n")
    return "logged"

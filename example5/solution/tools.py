...
@tool
def get_network_risk(msisdn: str) -> dict:
    """Return a simple network risk signal for a given MSISDN."""
    data_path = Path(__file__).parent / "data" / "network_risk.txt"
    for parts in _read_kv_lines(data_path):
        if len(parts) != 2:
            continue
        row_msisdn, risk = parts
        if row_msisdn == msisdn:
            return {"network_risk": risk}
    return {"network_risk": "unknown"}


@tool
def score_risk(profile: dict, events: dict, extra: dict) -> dict:
    ...
    if events.get("failed_logins_24h", 0) >= 3:
        score += 10
    if extra.get("network_risk") == "high":
        score += 10
    band = "low"
    if score >= 70:
        band = "high"

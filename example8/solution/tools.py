...
@tool
def write_failure_report(msisdn: str, reason: str) -> str:
    """Write a failure report to a local file."""
    report_path = Path(__file__).parent / "data" / "failure_reports.txt"
    line = f"{msisdn}|{reason}\n"
    report_path.write_text(report_path.read_text() + line if report_path.exists() else line)

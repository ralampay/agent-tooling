...
from tools import call_fraud_api, escalate_to_human, update_case_record, write_failure_report
...
class FraudAgent:
    def __init__(self):
        ...
        self.agent = Agent(
            model=self.model,
            tools=[call_fraud_api, update_case_record, write_failure_report, escalate_to_human],
            system_prompt=(
                "You are a failure-first fraud check agent.\n"
                "Always call call_fraud_api(msisdn).\n"
                "If ok=false, call write_failure_report(msisdn, reason) and then call escalate_to_human.\n"
    ...
    def run(self, user_input: str):
        ...
        if msisdn_match:
            ...
            self.agent.state["plan"] = [
                "call api",
                "validate",
                "update record or report failure",
            ]
